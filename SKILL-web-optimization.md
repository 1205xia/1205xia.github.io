...
## 开场封面与入场动画
目标：黑色、神秘、克制的单扇门入口，点击后进入个人主页。
1. 首屏全屏黑色背景，中央是一扇完整的黑色单门。
2. 只能从门轴方向打开一道门缝，禁止左右两扇门同时打开。
3. 不要门把手、星星、蝴蝶、流沙、光圈或装饰性跳动圆环，除非用户重新明确要求。
4. 点击门后用透视、`transform-origin` 和单扇门面产生门缝开启效果。
5. 门缝出现清晰克制的白色内光；门打开后白光短暂曝光，再切换主页。
6. 必须有静态首屏兜底，不依赖远程视频自动播放；微信和移动端视频失败时不能黑屏。
7. 内页可提供“再次入场”，返回入口并重新播放时间线。
8. 时间线分为等待、点击反馈、门缝打开、白光曝光、主页显现，时长适合网页访问。
